...
// A stale heartbeat is suspect, not proof of death (hq-txikg): it is only
// refreshed as a side effect of running a gt command, so a healthy agent inside
// a long tool call goes stale. It must be confirmed by an agent-liveness probe,
// and a failed probe must never be read as death.
