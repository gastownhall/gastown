...
func TestSchedulerMultiRigEpicAutoResolve(t *testing.T) {
	...
	// Link children to epic via depends-on (epic → child).
	// child1 is local to rig1 — resolves directly.
	addBeadDependencyOfType(t, epicID, child1, "depends-on", rig1Path)
	...
	addBeadDependencyOfType(t, epicID, child2ExtRef, "depends-on", rig1Path)
...
func TestSchedulerEpicFlagRejection(t *testing.T) {
	...
	addBeadDependencyOfType(t, epicID, child, "depends-on", rig1Path)
