...
| **C/C++ compiler** | current | `cc --version`, `c++ --version` | Required for embedded-capable source builds. The `gms_pure_go` build tag removes the ICU4C dependency. |
...
```bash
# Required
sudo dnf install -y git sqlite gcc gcc-c++
...
source instead, install `dolt` first, install `bd` with Go's
`gms_pure_go` tag, and ensure both
